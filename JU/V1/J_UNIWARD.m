...
% YY:
tEnd = toc(tStart);
fprintf(1, '%.2fs[Embedding simulation]\n', tEnd);
...
function [y, pChangeP1, pChangeM1] = EmbeddingSimulator(x, rhoP1, rhoM1, m)
    ...
    function lambda = calc_lambda(rhoP1, rhoM1, message_length, n)
        ...
        % YY:
        tInnerEnd = toc(tInnerStart);tInnerStart = tic;
        fprintf(1, '%.2f[calc_lambda:l3=%2.f]\n', tInnerEnd, l3);
        ...
        % YY:
        tInnerEnd = toc(tInnerStart);
        fprintf(1, '%.2f[calc_lambda:l1=%2.f]\n', tInnerEnd, l1);
    end

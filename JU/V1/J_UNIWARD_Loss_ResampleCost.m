function [rhoB, rhoM1, rhoP1, nzAC] = J_UNIWARD_Loss_ResampleCost(coverPath, nLevel)
...
sz = size(C_SPATIAL);org_k = sz(1); org_l = sz(2);
...
[C,S] = wavedec2(C_SPATIAL,nLevel,'db4');
C_SPATIAL = appcoef2(C,S,'db4');
...
sz = size(C_SPATIAL); k = sz(1); l = sz(2);
...
% [X, Y] = meshgrid(1:l, 1:k); [Xq, Yq] = meshgrid(1:org_l, 1:org_k);
% rhoQ = interp2(X,Y,rho,Xq,Yq, 'spline');
% figure
% subplot(1, 2, 1); surf(X,Y,rho); 
% subplot(1, 2, 2); surf(Xq,Yq,rhoQ); 
% rho = rhoQ;
rho = imresize(rho, [org_k, org_l]);
rhoM1 = rho;
rhoP1 = rho;
rhoB = rho;
...
% YY:
tEnd = toc(tStart);
fprintf(1, '%.2fs[Restore cost Matrix]\n', tEnd);
